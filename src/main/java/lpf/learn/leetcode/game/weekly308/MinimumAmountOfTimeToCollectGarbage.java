...
public class MinimumAmountOfTimeToCollectGarbage {
    public int garbageCollection(String[] garbage, int[] travel) {
        ...
        boolean haveM = false, haveP = false, haveG = false;

        for (int i = garbage.length - 1; i > 0; i--) {
            int[] parse = parse(garbage[i]);

            result += parse[0];
            result += parse[1];
            result += parse[2];

            haveM = haveM || parse[0] != 0;
            haveP = haveP || parse[1] != 0;
            haveG = haveG || parse[2] != 0;
