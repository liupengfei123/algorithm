...
public class DesignANumberContainerSystem {
    ...
    private Map<Integer, SortedSet<Integer>> numToIndex;
    ...
    public void change(int index, int number) {
        Integer oldNumber = indexToNum.put(index, number);
        if (oldNumber != null) {
            Set<Integer> temp = numToIndex.get(oldNumber);
            if (temp != null) {
                temp.remove(index);
            }
        }
        SortedSet<Integer> set = numToIndex.computeIfAbsent(number, k -> new TreeSet<>());
        set.add(index);
    }

    public int find(int number) {
        SortedSet<Integer> set = numToIndex.get(number);
