...
public class MeetingRoomsIii {
    ...
    public int mostBooked(int n, int[][] meetings) {
        ...
        PriorityQueue<Node> running = new PriorityQueue<>(Comparator.comparing((Node node) -> node.timed)
                .thenComparing((Node node) -> node.index));
        PriorityQueue<Node> free = new PriorityQueue<>(Comparator.comparing((Node node) -> node.timed)
                .thenComparing((Node node) -> node.index));

        for (int i = 0; i < n; i++) {
            free.offer(new Node(0, i));
        }

        List<Node> temp = new ArrayList<>();

        for (int[] meeting : meetings) {
            while (!running.isEmpty() && running.peek().timed <= meeting[0]) {
                temp.add(running.poll());
            }
            for (Node node : temp) {
                node.timed = 0;
                free.offer(node);
            ...
            poll.timed = Math.max(poll.timed, meeting[0]) + meeting[1] - meeting[0];
    ...
    class Node {
        long timed;
        int index;
        public Node(int timed, int index) {
            this.timed = timed;
            this.index = index;
        }
    }
