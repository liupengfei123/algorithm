...
public class AmountOfTimeForBinaryTreeToBeInfected {
    public int amountOfTime(TreeNode root, int start) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for (Integer node : map.getOrDefault(poll, new HashSet<>())) {
    ...
    public void dfs(TreeNode node, Map<Integer, Set<Integer>> map) {
        if (node.left != null) {
            Set<Integer> set = map.computeIfAbsent(node.val, key -> new HashSet<>());
            set.add(node.left.val);

            set = map.computeIfAbsent(node.left.val, key -> new HashSet<>());
            set.add(node.val);
        ...
        if (node.right != null) {
            ...
            set = map.computeIfAbsent(node.right.val, key -> new HashSet<>());
            set.add(node.val);
