...
public class LongestNonDecreasingSubarrayFromTwoArrays {
    ...
    public int maxNonDecreasingLength(int[] nums1, int[] nums2) {
        ...
        for (int i =1; i < n; i++) {
            ...
            if (nums1[i] >= nums1[i - 1]) temp1 = Math.max(temp1, v1);
            if (nums1[i] >= nums2[i - 1]) temp1 = Math.max(temp1, v2);

            int temp2 = 0;
            if (nums2[i] >= nums1[i - 1]) temp2 = Math.max(temp2, v1);
            if (nums2[i] >= nums2[i - 1]) temp2 = Math.max(temp2, v2);

            v1 = temp1 + 1;
            v2 = temp2 + 1;
            res = Math.max(res, Math.max(v1, v2));
        }
        return res;
    }
}
