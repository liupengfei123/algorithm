...
public class NumberOfPeopleAwareOfASecret {
    ...
    public int peopleAwareOfSecret(int n, int delay, int forget) {
        ...
        for (int i = 1; i <= n; i++) {
            result = (result + forgetArray[i]) % mod;
            result = (result + people[i]) % mod;

            if (people[i] <= 0) {
                continue;
            }
            for (int j = i + delay; j < i + forget && j <= n; j++) {
                people[j] = (people[j] + people[i]) % mod;
                if (j + forget <= n) {
                    forgetArray[j + forget] = (forgetArray[j + forget] - people[i]) % mod;
                }
            }
