    ...
    public int[] minReverseOperations(int n, int p, int[] banned, int k) {
        ...
        for (int step = 0; !q.isEmpty(); step++) {
            ...
            for (Integer i : temp) {
                ...
                int mn = Math.max(i - k + 1, k - 1 - i);
                int mx = Math.min(i + k - 1, 2 * n - k - i - 1);

                TreeSet<Integer> set = sets[mn & 1];
                for (int j = set.ceiling(mn); j <= mx; j = set.ceiling(mn)) {
                    q.add(j);
                    set.remove(j);
                }
            }
