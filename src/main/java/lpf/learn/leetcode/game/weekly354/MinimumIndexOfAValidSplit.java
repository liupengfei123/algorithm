...
public class MinimumIndexOfAValidSplit {
    ...
    public int minimumIndex(List<Integer> nums) {
        ...
        for (int i = m - 1; i >= 0; i--) {
            Integer merge = cnt.merge(nums.get(i), 1, Integer::sum);
            if ((merge << 1) > (m - i)) suff[i] = nums.get(i);

            if(suff[i + 1] == 0) continue;
            merge = cnt.get(suff[i + 1]);
            if (merge != null && (merge << 1) > (m - i)) suff[i] = suff[i + 1];
        }
        ...
        for (int i = 0; i < m; i++) {
            Integer merge = cnt.merge(nums.get(i), 1, Integer::sum);
            if ((merge << 1) > (i + 1)) {
                if (nums.get(i) == suff[i + 1]) return i;
                pre[i + 1] = nums.get(i);
            }

            if(pre[i] == 0) continue;
            merge = cnt.get(pre[i]);
            if (merge != null && (merge << 1) > (i + 1)) {
                if (pre[i] == suff[i + 1]) return i;
                pre[i + 1] = pre[i];
            }
        }
        return -1;
