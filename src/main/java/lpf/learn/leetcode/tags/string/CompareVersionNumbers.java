...
public class CompareVersionNumbers {
    public int compareVersion(String version1, String version2) {
        int[] ints1 = help(version1);
        int[] ints2 = help(version2);

        int count = Math.max(ints1.length, ints2.length);

        for (int i = 0; i < count; i++) {
            int i1 = i < ints1.length ? ints1[i] : 0;
            int i2 = i < ints2.length ? ints2[i] : 0;
    ...
    private int[] help(String version) {
