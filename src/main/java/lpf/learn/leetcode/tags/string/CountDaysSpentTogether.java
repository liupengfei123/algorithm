...
public class CountDaysSpentTogether {
    public int countDaysTogether(String arriveAlice, String leaveAlice, String arriveBob, String leaveBob) {
        ...
        if (leave.compareTo(arrive) < 0) return 0;

        int[] days = new int[]{0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};

        int am = Integer.parseInt(arrive.substring(0, 2));
        int ad = Integer.parseInt(arrive.substring(3, 5));
        int lm = Integer.parseInt(leave.substring(0, 2));
        int ld = Integer.parseInt(leave.substring(3, 5));

        int res = days[am] - ad + 1;

        for (int i = am + 1; i <= lm; i++) res += days[i];
