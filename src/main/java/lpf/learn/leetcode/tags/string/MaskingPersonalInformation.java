...
public class MaskingPersonalInformation {
    public String maskPII(String s) {
        if (s.contains("@")) return maskMail(s);
        return maskPhone(s);
    }
    ...
    private String maskPhone(String phone) {
        ...
        String res;
        switch (sb.length()) {
            case 10: res = "***-***-" + sb.substring(sb.length() - 4); break;
            case 11: res = "+*-***-***-" + sb.substring(sb.length() - 4); break;
            case 12: res = "+**-***-***-" + sb.substring(sb.length() - 4); break;
            default: res = "+***-***-***-" + sb.substring(sb.length() - 4);
        }
        return res;
    }
}
