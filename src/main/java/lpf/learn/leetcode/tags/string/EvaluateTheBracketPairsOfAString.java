...
public class EvaluateTheBracketPairsOfAString {
    public String evaluate(String s, List<List<String>> knowledge) {
        ...
        for (List<String> keyValue : knowledge) map.put(keyValue.get(0), keyValue.get(1));

        char[] chars = s.toCharArray();

        StringBuilder res = new StringBuilder(chars.length);
        for (int i = 0, bracket = -1; i < chars.length; i++) {
            char c = chars[i];
            if (c == ')') {
                res.append(map.getOrDefault(s.substring(bracket + 1, i), "?"));
                bracket = -1;
            } else if (c == '(') {
                bracket = i;
            } else if (bracket == -1) {
