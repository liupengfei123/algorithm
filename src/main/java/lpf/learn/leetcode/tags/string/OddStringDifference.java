...
public class OddStringDifference {
    public String oddString(String[] words) {
        long diff1 = help(words[0]);
        long diff2 = help(words[1]);

        if (diff1 == diff2) {
            for (int i = 2; i < words.length; i++) {
                if (help(words[i]) != diff1) return words[i];
            }
        }
        return diff1 == help(words[2]) ? words[1] : words[0];
    }

    private long help(String word) {
        long res = 0;
        for (int i = 1; i < word.length(); i++) {
            res = res * 13331 + (word.charAt(i) - word.charAt(i - 1)) * 13331;
        }
        return res;
    }
}
