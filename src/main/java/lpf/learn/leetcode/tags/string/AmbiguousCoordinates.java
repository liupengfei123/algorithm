...
public class AmbiguousCoordinates {
    public List<String> ambiguousCoordinates(String s) {
        ...
        for (int i = 2; i < s.length() - 1; i++) {
            help(s.substring(1, i), s.substring(i, s.length() - 1), result);
        }
        return result;
    }

    private void help(String s1, String s2, List<String> result) {
        List<String> lt = getPos(s1);
        if (lt.isEmpty()) return;

        List<String> rt = getPos(s2);
        if (rt.isEmpty()) return;

        for (String i : lt) {
            for (String j : rt) result.add("(" + i + ", " + j + ")");
        }
    }

    public List<String> getPos(String s) {
        List<String> pos = new ArrayList<>();
        if (s.charAt(0) != '0' || "0".equals(s)) pos.add(s);

        if (s.charAt(s.length() - 1) == '0') return pos;

        for (int p = 1; p < s.length(); ++p) {
            if ((p != 1 && s.charAt(0) == '0')) continue;
