...
public class MaximizeTheMinimumPoweredCity {
    public long maxPower(int[] stations, int r, int k) {
        ...
        long light = 0, right = 2000_000_000_000L;
        while (light < right) {
            long mid = light + (right - light + 1) / 2;

            if (check(diff, n, r, k, mid)) {
                light = mid;
            } else {
                right = mid - 1;
            }
    ...
    private boolean check(long[] diff, int n, int r, int k, long x) {
        long[] temp = new long[n + 1];
        System.arraycopy(diff, 0, temp, 0, n + 1);
