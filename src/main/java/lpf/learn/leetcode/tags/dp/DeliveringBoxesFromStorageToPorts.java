...
public class DeliveringBoxesFromStorageToPorts {
    public int boxDelivering(int[][] boxes, int portsCount, int maxBoxes, int maxWeight) {
        int n = boxes.length;
        int[] preW = new int[n + 1], neg = new int[n + 1];

        for (int i = 0; i < n; i++) {
            preW[i + 1] = preW[i] + boxes[i][1];

            if (i > 0) neg[i + 1] = neg[i] + (boxes[i - 1][0] != boxes[i][0] ? 1 : 0);
        }
        ...
        for (int i = 1; i <= n; i++) {
            while (i - queue.peekFirst() > maxBoxes || preW[i] - preW[queue.peekFirst()] > maxWeight) {
                queue.pollFirst();
            }

            dp[i] = neg[i] + g[queue.peekFirst()] + 2;

            if (i != n) {
                g[i] = dp[i] - neg[i + 1];
                while (!queue.isEmpty() && g[queue.peekLast()] >= g[i]) {
                    queue.pollLast();
                }
                queue.offerLast(i);
            }
