...
public class ShoppingOffers {
    ...
    public int shoppingOffers(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
        ...
        this.specials = new int[special.size()][n + 1];

        this.dp = new HashMap<>();

        int need = 0;
        for (int i = 0; i < n; i++) {
            this.prices[i] = price.get(i);
            need |= needs.get(i) << (i * 4);
        }

        for (int i = 0; i < special.size(); i++) {
            for (int j = 0; j <= n; j++) specials[i][j] = special.get(i).get(j);
        }

        return help(need);
    }

    private int help(int need) {
        Integer value = dp.get(need);
        if (value != null) return value;

        int res = notUsedSpecial(need);

        for (int[] special : specials) {
            boolean flag = true;
            for (int i = 0; i < n; i++) {
                if (((need >> (i * 4)) & 0xF) < special[i]) {
                    flag = false;
                    break;
                }
            }
            ...
            int newNeed = 0;
            for (int i = 0; i < n; i++) {
                int temp = ((need >> (i * 4)) & 0xF) - special[i];
                newNeed |= temp << (i * 4);
            }
            res = Math.min(res, special[n] + help(newNeed));
        }
        dp.put(need, res);
        return res;
    }

    private int notUsedSpecial(int need) {
        int res = 0;
        for (int i = 0; i < n; i++) {
            res += ((need >> (i * 4)) & 0xF) * prices[i];
        }
        return res;
    }
}
