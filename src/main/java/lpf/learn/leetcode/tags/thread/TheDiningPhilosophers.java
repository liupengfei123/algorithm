...
public class TheDiningPhilosophers {
    ...
    private synchronized boolean canPickFork(int philosopher) {
        if (flag == 31) {
            flag = 0;
        }

        if ((flag & (1 << philosopher)) > 0) {
            return false;
        }
    ...
    private synchronized void putFork(int philosopher) {
        flag |= 1 << philosopher;
