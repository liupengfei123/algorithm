...
public class FrogPositionAfterTSeconds {
    ...
    public double frogPosition(int n, int[][] edges, int t, int target) {
        this.target = target;
        this.childIndex = new List[n + 1];
        this.value = 1;
        this.targetDepth = 1;

        for (int i = 1; i <= n; i++) childIndex[i] = new ArrayList<>();

        childIndex[1].add(0);
        for (int[] edge : edges) {
            childIndex[edge[0]].add(edge[1]);
            childIndex[edge[1]].add(edge[0]);
        }

        dfs(1, 0, 0);

        if (targetDepth > t) return 0;
        if (targetDepth < t && childIndex[target].size() > 1) return 0;

        return 1D / value;
    }

    private boolean dfs(int i, int depth, int pi) {
        if (i == target) {
        ...
        boolean res = false;
        if (childIndex[i] != null) {
            for (Integer ni : childIndex[i]) {
                if (ni != pi) res |= dfs(ni, depth + 1, i);
            }
        }
        if (res) value *= (childIndex[i].size() - 1);
        return res;
    }
}
