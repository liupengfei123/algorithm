...
public class MaximumDifferenceBetweenNodeAndAncestor {
    ...
    public int dfsDiff(TreeNode node, int mi, int mx) {
        if (node == null) return 0;

        int val = node.val;
        int diff = Math.max(Math.abs(val - mi), Math.abs(val - mx));

        mi = Math.min(mi, val);
        mx = Math.max(mx, val);

        diff = Math.max(diff, dfsDiff(node.left, mi, mx));
        diff = Math.max(diff, dfsDiff(node.right, mi, mx));
