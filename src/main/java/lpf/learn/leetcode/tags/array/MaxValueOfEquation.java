...
public class MaxValueOfEquation {
    public int findMaxValueOfEquation(int[][] points, int k) {
        PriorityQueue<int[]> queue = new PriorityQueue<>(Comparator.comparing((int []a) -> a[1]).reversed());

        int n = points.length, res = Integer.MIN_VALUE;
        for (int i = 0, j = 0; i < n; i++) {
            while (j < n && points[j][0] - points[i][0] <= k) {
                queue.add(new int[]{j, points[j][0] + points[j][1]});
                j++;
            }
            ...
            if (!queue.isEmpty()) {
                res = Math.max(res, queue.peek()[1] + points[i][1] - points[i][0]);
            }
        }
        return res;
    }
