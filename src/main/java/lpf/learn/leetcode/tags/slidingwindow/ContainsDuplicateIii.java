...
public class ContainsDuplicateIii {
    public boolean containsNearbyAlmostDuplicate(int[] nums, int indexDiff, int valueDiff) {
        int n = nums.length, k = indexDiff + 1;
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (floor != null && Math.abs(floor - val) <= valueDiff) return true;

            Integer ceiling = treeSet.ceiling(val);
            if (ceiling != null && Math.abs(ceiling - val) <= valueDiff) return true;
