...
public class MovingStonesUntilConsecutiveIi {
    public int[] numMovesStonesII(int[] stones) {
        ...
        return new int[]{getMin(stones, n), max};
    }

    private int getMin(int[] stones, int n) {
        int min = n;
        for (int l = 0, r = 0; r < n; r++) {
            while (stones[l] + n <= stones[r]) {
                l++;
            }
        ...
        if (min == 1) {
            if (stones[n - 2] - stones[0] == n - 2 && stones[n - 1] - stones[n - 2] > 2) min = 2;
            if (stones[n - 1] - stones[1] == n - 2 && stones[1] - stones[0] > 2) min = 2;
        }
        return min;
    }
}
