...
public class MinimumNumberOfOperationsToMakeArrayContinuous {
    public int minOperations(int[] nums) {
        ...
        for (int j = 0; j < nums.length; j++) {
            if (nums[j] != lastV) nums[i++] = lastV = nums[j];
        }

        int len = nums.length, n = i, res = len - 1;
        for (int l = 0, r = 1; r < n; r++) {
            while (l < r && nums[r] - nums[l] >= len) l++;
    ...
    public int minOperations2(int[] nums) {
        ...
        int i = 0, lastV = 0;
        for (int j = 0; j < nums.length; j++) {
            if (nums[j] == lastV) continue;

            nums[i++] = lastV = nums[j];
        }

        int len = nums.length, n = i, res = len - 1, diff = 0;

        for (int l = 0, r = 1; r < n; r++) {
            diff += Math.abs(nums[r] - nums[r - 1] - 1);

            while (l < r && len - (r - l + 1) < diff) {
                diff -= Math.abs(nums[l + 1] - nums[l] - 1);
                l++;
            }
            res = Math.min(res, len - (r - l + 1));
        }
