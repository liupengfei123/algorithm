...
public class MaximumSumOfTwoNonOverlappingSubarrays {
    public int maxSumTwoNoOverlap(int[] nums, int firstLen, int secondLen) {
        ...
        for (int i = secondLen; i < n; i++) {
            sumS1 += nums[i];
            sumS2 += nums[n - i - 1];
            sumS1 -= nums[i - secondLen];
            sumS2 -= nums[n - i - 1 + secondLen];

            dp[i][0] = Math.max(dp[i - 1][0], sumS1);
            dp[n - i - 1][1] = Math.max(dp[n - i][1], sumS2);
        }

        int sumF = 0;
        for (int i = 0; i < firstLen; i++) sumF += nums[i];

        int res = sumF + dp[firstLen][1];
        for (int i = firstLen; i < n; i++) {
            sumF += nums[i];
            sumF -= nums[i - firstLen];

            if (i < n - secondLen) res = Math.max(res, sumF + dp[i + 1][1]);

            if (i >= firstLen + secondLen) res = Math.max(res, sumF + dp[i - firstLen][0]);
        }
        return res;
    }
}
