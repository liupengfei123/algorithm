...
public class ExpressiveWords {
    public int expressiveWords(String s, String[] words) {
        ...
        for (String word : words) {
            if (solve(sc, n, word.toCharArray(), word.length()))
                res++;
        }
        return res;
    }

    private boolean solve(char[] sc, int n, char[] wc, int m) {
        int i = 0, j = 0;

        while (i < n && j < m) {
            if (sc[i] != wc[j]) return false;

            int ti = i, tj = j;
            while (i < n && sc[ti] == sc[i]) i++;
            while (j < m && wc[tj] == wc[j]) j++;
