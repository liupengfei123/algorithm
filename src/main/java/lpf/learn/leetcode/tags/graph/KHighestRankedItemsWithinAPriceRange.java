...
public class KHighestRankedItemsWithinAPriceRange {
    ...
    private final PriorityQueue<Integer[]> res = new PriorityQueue(Comparator
            .comparing((Integer[] a) -> a[2])
            .thenComparing((Integer[] a) -> a[3])
            .thenComparing((Integer[] a) -> a[0])
    ...
    public List<List<Integer>> highestRankedKItems(int[][] grid, int[] pricing, int[] start, int k) {
        ...
        int cnt = Math.min(k, res.size());
        while (cnt-- > 0) {
            Integer[] poll = res.poll();
            ans.add(Arrays.asList(poll[0], poll[1]));
        }
        return ans;
    }
    ...
    private void bfs() {
        while (!queue.isEmpty()) {
            ...
            if (pricing[0] <= grid[i][j] && grid[i][j] <= pricing[1])
                res.add(new Integer[]{i, j, x, grid[i][j]});

            for (int[] d : dict) {
                int ni = i + d[0], nj = j + d[1];
                if (ni < 0 || ni >= n || nj < 0 || nj >= m || grid[ni][nj] == 0 || visited[ni][nj]) continue;
