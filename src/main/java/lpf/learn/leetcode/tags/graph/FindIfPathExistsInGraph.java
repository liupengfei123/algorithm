...
public class FindIfPathExistsInGraph {
    ...
    private HashMap<Integer, Set<Integer>> path;
    ...
    public boolean validPath(int n, int[][] edges, int source, int destination) {
        ...
        for (int[] edge : edges) {
            Set<Integer> set = path.getOrDefault(edge[0], new HashSet<>());
            set.add(edge[1]);
            path.putIfAbsent(edge[0], set);

            set = path.getOrDefault(edge[1], new HashSet<>());
            set.add(edge[0]);
            path.putIfAbsent(edge[1], set);
        }
    ...
    private boolean dfs(int source, int destination) {
        ...
        Set<Integer> set = path.getOrDefault(source, Collections.<Integer>emptySet());
